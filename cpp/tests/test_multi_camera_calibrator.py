...
if __name__ == '__main__':
    ...
    for root, dirs, files in os.walk(camera_folder):
        ...
        pinhole_cam_list = [
            xr.camera.PinholeCameraParameter() for i in range(camera_cnt)
        ]
    ...
    for root, dirs, files in os.walk(image_folder):
        for file in files:
            ...
            matched = re.match(
                r'img(?P<img_index>\d+)_cam(?P<camera_index>\d+).jpg', file)
        ...
        for file in files:
            ...
            matched = re.match(
                r'img(?P<img_index>\d+)_cam(?P<camera_index>\d+).jpg', file)
            frames[int(matched.group('img_index'))][int(
                matched.group('camera_index'))] = img
    ...
    for idx, pin in enumerate(vc):
        out_path = f'outputs/{idx}.json'
        os.makedirs(os.path.dirname(out_path), exist_ok=True)
        print(f'save file: {out_path}')
        pin.SaveFile(out_path)
