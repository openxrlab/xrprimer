...
input_dir = 'test/data/test_ops/test_projection'
output_dir = 'test/data/output/test_ops/test_projection'
...
def test_opencv_projector():
    ...
    for cam_idx in range(n_view):
        cam_param_path = os.path.join(input_dir, f'cam_{cam_idx:02d}.json')
    ...
    canvas = np.ones(
        shape=[n_view, cam_param.height, cam_param.width, 3], dtype=np.uint8)
    for cam_idx in range(n_view):
