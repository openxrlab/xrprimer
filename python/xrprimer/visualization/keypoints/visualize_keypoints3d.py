...
from xrprimer.data_structure.camera import PinholeCameraParameter, FisheyeCameraParameter
...
def visualize_projected_keypoints3d(
    # input args
    keypoints: Keypoints,
    ...
    """Visualize multi-frame keypoints3d by OpenCV, overlay with 2D images.
    For plot args, please
    ...
    projected_kps2d = projector.project(
        points=keypoints.get_keypoints().reshape(n_frame*n_person*n_kps, 3),
        points_mask=keypoints.get_mask().reshape(n_frame*n_person*n_kps, 1)
    )
    keypoints2d = keypoints.clone()
    # mask is not changed, set keypoints data is enough
    keypoints2d.set_keypoints(projected_kps2d.reshape(n_frame, n_person, n_kps, 3))
