...
class ViewerState:
    ...
    def render_image_in_viewer(self) -> None:
        ...
        color = (10, 20, 20)
        cv.putText(img, "translation: " + str(camera_translation), (10, 100), font, 1, color, 1)
        cv.putText(img, "rotation: " + str(camera_rotation), (10, 200), font, 1, color, 1)
        cv.putText(img, "fov: " + str(camera_fov), (10, 300), font, 1, color, 1)
        cv.putText(img, "render type: " + str(render_type), (10, 400), font, 1, color, 1)
        scaled_resolution = self.get_resolution()
        cv.putText(img, f"resolution: {scaled_resolution[0]}x{scaled_resolution[1]}", (10, 500), font, 1, color, 1)
        scaled_image = cv.resize(img, scaled_resolution, interpolation=cv.INTER_AREA)
        ...
        data = str(f"data:image/{image_format};base64," + base64.b64encode(data).decode("ascii"))
