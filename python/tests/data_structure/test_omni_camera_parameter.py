...
output_dir = 'tests/data/output/data_structure/test_omni_camera_parameter'
eps = 1e-4
...
def test_load():
    ...
    with pytest.raises(ValueError):
        camera_parameter.load(
            os.path.join(input_dir, 'pinhole_param_at_origin.json'))
    camera_parameter = OmniCameraParameter.fromfile(input_json_path)
    assert camera_parameter.get_intrinsic(k_dim=4)[0][0] - focal_length_x < eps
